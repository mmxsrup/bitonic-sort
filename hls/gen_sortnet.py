	...
	with open(filename, 'a') as f:
		...
		while (block <= DATA_SIZE):
			...
			while (step >= 1):
				...
				s = 'void block%d_step%d_net(ap_int<DATA_W> datas[DATA_SIZE]) {\n' % (block, step)
				f.write(s)
		...
		f.write('\nvoid sorting_network(ap_int<DATA_W> datas[DATA_SIZE]) {\n')
...
def gen_header(filename_header):
	with open(filename_header, 'a') as f:
		...
		f.write('void sorting_network(ap_int<DATA_W> datas[DATA_SIZE]);\n')
		f.write('\n#endif\n')
